...
namespace AnotadorGymApp.Data.Models.Entities
{
    public class Rutinas : INotifyPropertyChanged
    {
        #region Gets
        [NotMapped]
        public bool TieneImagen => !string.IsNullOrWhiteSpace(ImageSource);
        [NotMapped]
        ...
        public ICollection<RutinaSemana> Semanas { get; set; } = new ObservableCollection<RutinaSemana>();
    ...
    public class RutinaSemana : INotifyPropertyChanged, ISeleccionable
    {
        public int SemanaId { get; set; }
        public int RutinaId { get; set; }
        public Rutinas Rutina { get; set; }
        public ICollection<RutinaDia> Dias { get; set; } = new ObservableCollection<RutinaDia>();
        public bool Completado { get; set; } = false;
    ...
    public class RutinaDia : INotifyPropertyChanged, ISeleccionable
    {             
        ...
        public ICollection<RutinaEjercicio> Ejercicios { get; set; } = new ObservableCollection<RutinaEjercicio>();
        [NotMapped]
        private bool seleccionado;
        [NotMapped]
        public bool Seleccionado
    ...
    public class RutinaEjercicio : INotifyPropertyChanged, ISeleccionable
    {
        ...
        private ObservableCollection<RutinaSeries> _series = new ObservableCollection<RutinaSeries>();
        public ObservableCollection<RutinaSeries> Series
        {
            get => _series;
            set
            {
                ...
                _series = value ?? new ObservableCollection<RutinaSeries>();
        ...
        {
            Debug.WriteLine("CollectionChanged disparado");
            if (e.NewItems != null)
            {
                foreach (RutinaSeries s in e.NewItems)
                    s.PropertyChanged += Serie_PropertyChanged;
            }

            if (e.OldItems != null)
            {
                foreach (RutinaSeries s in e.OldItems)
                    s.PropertyChanged -= Serie_PropertyChanged;
            }
            OnPropertyChanged(nameof(SeriesTotales));
            OnPropertyChanged(nameof(ProgresoSeriesFormateado));
        ...
        [NotMapped]
        public int SeriesTotales => Series?.Count ?? 0;
        [NotMapped]
        public int SeriesCompletadas => Series?.Count(s => s.EstadoSerie == 3) ?? 0;
        [NotMapped]        
    ...
    public class RutinaSeries : INotifyPropertyChanged, ISeleccionable
    {
        ...
        [NotMapped]
        public RegistroSerie SetLog { get; set; } = new RegistroSerie();
